...
import { computeUserAnalytics, computeUserDetail } from '@/lib/user-analytics';
...
export async function GET(req: NextRequest) {
  ...
  if (user) {
    const detail = computeUserDetail(user);
    if (!detail) return NextResponse.json({ error: '用户不存在' }, { status: 404 });
    return NextResponse.json(detail);
  }

  return NextResponse.json(computeUserAnalytics());
}
