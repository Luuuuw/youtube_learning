...
log_file = os.path.join(LOGS_DIR, f"download_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if info is None:
                    last_error = "yt-dlp 返回空信息（视频可能不可用或地区限制）"
                ...
                logger.info(f"  口音分类: {accent}")
                logger.info(f"  保存路径: {output_dir}")

                all_files = os.listdir(output_dir)
                vtt_files = [f for f in all_files if f.endswith('.vtt')]
                if vtt_files:
                    logger.info(f"  字幕文件: {', '.join(vtt_files)}")
                else:
                    logger.warning("  未找到字幕文件（该视频可能没有自动字幕）")
            ...
            if any(kw in error_lower for kw in ['429', 'too many requests', 'rate limit']):
                ...
                logger.error(f"视频不可用（私有/下架/版权）: {youtube_url} - {e}")
                return None
            else:
...
def batch_download(url_file):
    ...
    logger.info(f"下载完成: {success}/{len(urls)} 成功")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("=" * 50)
        print("  VibeEnglish 视频下载器")
        print("=" * 50)
        print()
        print("使用方式:")
        print("  1. 下载单个视频: python downloader.py <YouTube链接>")
        print("  2. 批量下载: python downloader.py urls.txt")
        print()
